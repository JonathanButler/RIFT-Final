...
if(isset($_POST['type'])) {
	$type = $_POST['type'];
	$product_list = "";
	$sql = mysql_query("SELECT * FROM Products WHERE gender='f' and type='$type' ORDER BY 	color");
...
elseif (isset($_POST['color'])) {
	$color = $_POST['color'];
	$product_list = "";
	$sql = mysql_query("SELECT * FROM Products WHERE gender='f' and color='$color' ORDER BY 	type");
	$productCount = mysql_num_rows($sql); //count the output
	if($productCount > 0){
 	while($row=mysql_fetch_array($sql)){
 		$id = $row["pid"];
...
elseif (isset($_POST['price'])) {
	$price = $_POST['price'];
	$product_list = "";
	$sql = mysql_query("SELECT * FROM Products WHERE gender='f' and price <= $price ORDER BY 	type");
	$productCount = mysql_num_rows($sql); //count the output
	if($productCount > 0){
 	while($row=mysql_fetch_array($sql)){
 		$id = $row["pid"];
...
<?php include_once("riftindexhead.php");?>
				...
				<br/>
				<h3>RIFT Products for Women:</h3>
	<br/>
				Browse:
				<table>
				<tr>
				<form action="shopwomen.php" method="POST">
